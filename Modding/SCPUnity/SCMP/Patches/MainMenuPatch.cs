...
internal class MainMenuPatch
{
    ...
    // Button background image is loaded late
    private static void CreateMultiplayerMenu()
    {
        ...
        GameObject copyObject = _instance.transform.GetChild(0).GetChild(2).GetChild(1).gameObject;
        ...
        IpTextObject =
            Object.Instantiate(_instance.transform.GetChild(0).GetChild(2).GetChild(2).GetChild(0).gameObject);
        ...
        GameObject navCopy = _instance.transform.GetChild(0).GetChild(2).GetChild(3).gameObject;
        ...
        HostGameScreenNavigationButtons = HostGameScreen.transform.GetChild(3).gameObject;
        ...
        HostGameScreen.transform.GetChild(1).gameObject.SetActive(false);
        HostGameScreenCancelButtonObject = HostGameScreenNavigationButtons.transform.GetChild(1).gameObject;
        HostGameScreenCancelButtonObject.name = "CancelButton";
        ...
        HostGameScreenSeed = HostGameScreen.transform.GetChild(2).GetChild(3)
            .GetChild(1).gameObject;

        // MULTIPLAYER MENU : HOST GAME SCREEN : PLAYER NAME
        string dBoyRandomNumber = $"D-{UnityEngine.Random.RandomRangeInt(1, 9999)}";
        HostScreenPlayerNameObject = HostGameScreen.transform.GetChild(2).GetChild(3).GetChild(0).gameObject;
        HostScreenPlayerNameInputField = HostScreenPlayerNameObject.transform.GetChild(0).GetComponent<TMP_InputField>();
        HostScreenPlayerNameInputField.text = dBoyRandomNumber;
        HostScreenPlayerNameInputFieldText = HostScreenPlayerNameObject.transform.GetChild(0).GetChild(0).GetChild(1).GetComponent<TextMeshProUGUI>();
        ...
        GameObject hostNavCopy = _instance.transform.GetChild(0).GetChild(2).GetChild(3).gameObject;
    ...
    private static void OnMultiplayerClicked()
    {
        ...
        // BACKGROUND IMAGE (loads late)
        MultiplayerBackgroundImage = Object.Instantiate(_backgroundImage);
        MultiplayerBackgroundImage.SetActive(true);
        MultiplayerBackgroundImage.transform.SetParent(_multiplayerMenuObject.transform);
        MultiplayerBackgroundImage.transform.position = _backgroundImage.transform.position;
        MultiplayerBackgroundImage.transform.localPosition = _backgroundImage.transform.localPosition;
        
        HostButtonObject.transform.localPosition = new Vector3(MultiplayerBackgroundImage.transform.localPosition.x,
            (Screen.height * 0.1f) + MultiplayerBackgroundImage.transform.localPosition.y, 0);
        JoinButtonObject.transform.localPosition = new Vector3(MultiplayerBackgroundImage.transform.localPosition.x,
            (Screen.height * -0.1f) + MultiplayerBackgroundImage.transform.localPosition.y, 0);
        IpInputFieldObject.transform.localPosition = new Vector3(MultiplayerBackgroundImage.transform.localPosition.x,
            IpTextObject.transform.localPosition.y - (Screen.height * 0.0425f), 0);
        PortInputFieldObject.transform.localPosition = new Vector3(MultiplayerBackgroundImage.transform.localPosition.x,
            PortTextObject.transform.localPosition.y - (Screen.height * 0.0425f), 0);
        NameInputFieldObject.transform.localPosition = new Vector3(MultiplayerBackgroundImage.transform.localPosition.x,
    ...
    private static void JoinHost()
    {
        ...
        // Check this after the above for username check
        if (!_isHost)
        {
            //GameObject playerCopy = HostGameScreenNavigationButtons.transform.GetChild(2).GetChild(0).gameObject;
            //GameObject player = Object.Instantiate(playerCopy);
            //player.transform.GetChild(0).GetComponent<TextMeshProUGUI>().text =
            //    username;
            //LobbyPlayers.Add(player);
        ...
        if (!_isHost)
        {
            ...
            Console.WriteLine($"playerNameString : {playerNameString}");

            foreach (string temp in playerNameString.Split(','))
            {
                string name = temp;
                while (name.Contains("\n"))
                {
                ...
                if (_newGameCopy)
                {
                    GameObject newPlayer =
                        Object.Instantiate(_newGameCopy.transform.GetChild(2).GetChild(0).gameObject);
                    newPlayer.name = name;
                    ...
                    newPlayer.transform.GetChild(0).GetComponent<TextMeshProUGUI>().enableWordWrapping = false;
                    LobbyPlayers.Add(newPlayer);
                }
                else
                {
                    Console.WriteLine($"_newGameCopy failed???");
                }
            }
        ...
        else
        {
            if (LobbyPlayers.Count > 0)
            {
                ...
                Console.WriteLine($"Writing [{playerNames.ToString()}]");
        ...
        //    new Thread(() =>
        //    {
        //        // check if a new player joined the lobby by checking if the lobbyinfo txt file changed
        //        string previousLobbyInfo = "";
        //        while (_inLobby)
        //        {
        //            helpers.WaitForFile(helpers.LobbyInfoFilePath, ref _inLobby, false, FileAccess.Read, FileShare.Write);
        //
        //            // TODO: not remake entire list each time someone joins
        //            string currentLobbyInfo = helpers.ReadFileBytes(helpers.LobbyInfoFilePath);
        //            if (!previousLobbyInfo.Equals(currentLobbyInfo))
        //            {
        //                LobbyPlayers.Clear();
        //
        //                foreach (string temp in currentLobbyInfo.Split(','))
        //                {
        //                    string name = temp;
        //                    while (name.Contains("\n"))
        //                    {
        ...
        //            }
        //
        //            previousLobbyInfo = currentLobbyInfo;
        //
        //            LoadLobbyPlayers();
        //        }
        //    }).Start();
